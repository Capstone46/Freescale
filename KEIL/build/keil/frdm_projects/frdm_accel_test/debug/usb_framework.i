#line 1 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.c"
...
#line 39 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.c"
...
#line 40 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.c"
#line 1 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.h"
...
#line 42 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.h"
#line 43 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.h"
...
#line 44 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.h"
...
#line 108 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.h"
...
#line 41 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.c"
#line 42 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.c"
#line 49 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.c"
...
static USB_CLASS_CALLBACK g_framework_callback=0;
 
static USB_REQ_FUNC g_other_req_callback=0;
...
boolean const g_validate_request[(13)][3] =
{
  {1,1,0},  
...
#line 99 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.c"
  {1,1,0},  
...
static uint_8 ext_req_to_host[32 + (8)];
 


 
USB_REQ_FUNC const g_standard_request[(13)] =
{
...
)
{
    ...
    error = _usb_device_register_service(controller_ID, (0x00),
...
)
{
    ...
    g_framework_callback = 0;

     
    g_other_req_callback = 0;

     
    error = _usb_device_unregister_service(&controller_ID, (0x00));

     
    return error;
}
#line 354 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.c"
...
#line 407 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.c"
...
)
{
    ...
    (void)_usb_device_get_status(&(event->controller_ID), (0x01),
        &device_state);

    if (event->setup == 1)
    {
         
        (void)memcpy(&g_setup_pkt, event->buffer_ptr, (8));

         
        g_setup_pkt.index = (g_setup_pkt . index);
        g_setup_pkt.value = (g_setup_pkt . value);
        g_setup_pkt.length = (g_setup_pkt . length);
        
         
        if ((g_setup_pkt.request_type & (0x60)) ==
            (0x00))
        {
             
            if (g_standard_request[g_setup_pkt.request] != 0)
            {
                 
                if((device_state < (0x03)) &&
        ...
        else  
        {
            ...
            size = (uint_16)g_setup_pkt.length;
            if (size <= 32) {
            	if( (size != 0) &&
            			((g_setup_pkt.request_type & (0x80)) ==
            			(0x00)) )
            	{
            		(void)memcpy(ext_req_to_host, &g_setup_pkt, (8));
            	...
            	else if(g_other_req_callback != 0) 
            	{
            		status = g_other_req_callback(event->controller_ID,
            				&g_setup_pkt, &data, (uint_16*)&size);
            	}
            }
            else
            {
                ...
                USB_Control_Service_Handler(event->controller_ID,
                	(0xCD), &g_setup_pkt, &data, (uint_16*)&size);            	
            	return;
            }
        }

        USB_Control_Service_Handler(event->controller_ID,
            status, &g_setup_pkt, &data, (uint_16*)&size);
    }
     
    else if(device_state == (0x04))
    {
            ...
            (event->direction == (0)) )
    {
        ...
        if(g_other_req_callback != 0)
        {    
            size = (uint_16)(event->len + (8));
            status = g_other_req_callback(event->controller_ID,
...
)
{
    ...
    else  
    {
        ...
        {
            *size = (uint_16)setup_packet->length;
        }

          
        (void)USB_Class_Send_Data(controller_ID,
            (0), *data, *size);
        if((setup_packet->request_type & (0x80)) ==
            (0x80))
        {    
...
{
	uint_8_ptr pu8ConfigDesc = 0;
	uint_16 u16DescSize;
    uint_8 interface, endpoint = 0;
    uint_8 status;
    uint_8 device_state = (0x03);


    status = _usb_device_get_status(&controller_ID, (0x01), &device_state);
  
    if((setup_packet->request_type & (0x03)) == (0x00))
    {
#line 633 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.c"
        {
         uint_8 device_state1;
        	
          
         status = _usb_device_get_status(&controller_ID, (0x06), &device_state1);
         g_std_framework_data = (uint_16)device_state1;
         g_std_framework_data &= (0x0003);
         g_std_framework_data = (g_std_framework_data);

          		
         status = USB_Desc_Get_Descriptor(controller_ID, (2),
        		                          0, 0, &pu8ConfigDesc, (uint_16 *)&u16DescSize);		
		
		 if((pu8ConfigDesc[7] & (0x40)) != 0)
		 {
			g_std_framework_data |= (0x0001);
		 }
		 else
		 {
			g_std_framework_data &= (~0x0001);
    ...
    else if((setup_packet->request_type & (0x03)) == (0x01))
    {
         
    	interface = (uint_8) setup_packet->index;
        if((device_state == (0x01)) && (interface > 0))
        {
        	 status = (0xCD);
        }
        else
    ...
    else if((setup_packet->request_type & (0x03)) == (0x02))
    {
         
    	if((device_state == (0x01)) && (endpoint > 0))
		{
			 status = (0xCD);
		}
    	else
...
)
{
    ...
    *size=0;
    ...
    set_request = (uint_8)((setup_packet->request & (0x02)) >> 1 );
    ...
    if((setup_packet->request_type & (0x03)) == (0x00))
    {
        ...
        {           
			...
			if((setup_packet->value == (0x0001)) || (setup_packet->value == (0x0002)))
            {
				status = _usb_device_get_status(&controller_ID, (0x06), &device_status);
				...
				{ 
					...
					status = _usb_device_set_status(&controller_ID, (0x06), device_status);
				}
            }
#line 774 "..\\..\\..\\..\\src\\drivers\\usbd\\usb_framework.c"
            else
            {
               status = (0xCD);
            }																					
    ...
    else if((setup_packet->request_type & (0x03)) == (0x02))
    {
	    ...
	    status = _usb_device_set_status(&controller_ID, (uint_8)(epinfo|(0x10)), set_request);
	    
	    event = (uint_8)(set_request ? (7) : (8));

	    if(setup_packet->request == 0x1)
	    {
...
)
{
    ...
    (void)_usb_device_set_status(&controller_ID, (0x03),
        g_assigned_address);
...
)
{
    ...
    *size = (1);
    (void)_usb_device_get_status(&controller_ID, (0x04), &device_status);
...
)
{
    ...
    *size = 0;
     ...
     
    {
        ...
        status = _usb_device_set_status(&controller_ID, (0x01),
            device_state);
        status = _usb_device_set_status(&controller_ID,
            (0x04), (uint_8)config_val);
        


 
        g_framework_callback(controller_ID,(1),
            (void *)&config_val);
        g_framework_callback(controller_ID,(2), 0);
    }
...
)
{
	...
	*size = (2);
	...
	status = _usb_device_get_status(&controller_ID, (0x05),
									&device_status);
	g_std_framework_data = (uint_16)device_status;
	*data = (uint_8_ptr)&g_std_framework_data;
...
{
    uint_8 type = ((uint_8)(setup_packet->value>>8));	
    uint_16 index = (uint_16)(0xffffffff);
    uint_8 str_num = (uint_8)(0xffffffff);
    uint_8 status;

	 
	index = setup_packet->index;
	 
	str_num = ((uint_8)setup_packet->value);
